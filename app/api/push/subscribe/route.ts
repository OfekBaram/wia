...
export async function POST(req: Request) {
  ...
  const { subscription, locale } = await req.json().catch(() => ({} as { subscription?: { endpoint?: string }; locale?: string }))
  ...
  const { error } = await admin.from('push_subscriptions').upsert(
    { user_id: userData.user.id, endpoint: subscription.endpoint, subscription, locale: locale === 'he' ? 'he' : 'en' },
    { onConflict: 'endpoint' },
  )
