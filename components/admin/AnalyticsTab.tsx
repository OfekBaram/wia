...
const GENDER_LABEL: Record<string, string> = {
  woman: 'Women', man: 'Men', 'non-binary': 'Non-binary', unspecified: 'Unspecified',
}
...
export function AnalyticsTab({ venueSlug }: Props) {
  ...
  if (!data) return <div className="py-12 text-center text-wia-ink/50 text-sm">Could not load analytics.</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <Stat icon={Users}       label="Total visitors"   value={data.totalVisitors}  color="text-wia-purple" />
        <Stat icon={Users}       label="Today"            value={data.visitorsToday}  color="text-wia-green"  />
        <Stat icon={Users}       label="This week"        value={data.visitorsWeek}   color="text-wia-cyan"   />
        <Stat icon={QrCode}      label="Total scans"      value={data.scanCount}      color="text-wia-pink"   />
        <Stat icon={TrendingUp}  label="Peak count"       value={data.peakCount}      color="text-wia-purple" />
        <Stat icon={Clock}       label="Avg session"      value={data.avgSessionMin !== null ? `${data.avgSessionMin}m` : '—'} color="text-wia-cyan" sub={data.avgSessionMin !== null ? undefined : 'No completed sessions yet'} />
        <Stat icon={Heart}       label="Total likes"      value={data.totalLikes}     color="text-wia-pink"   />
        <Stat icon={MessageCircle} label="Messages sent"  value={data.totalMessages}  color="text-wia-purple" />
        <Stat icon={BarChart2}   label="Avg age"          value={data.avgAge ?? '—'}  color="text-wia-cyan"   />
      </div>

      {/* Gender breakdown */}
      {totalGender > 0 && (
        <GlassCard className="p-5">
          <h3 className="font-display font-semibold text-wia-ink mb-4">Gender breakdown</h3>
          <div className="space-y-2.5">
            {Object.entries(data.genderCounts).sort(([,a],[,b]) => b - a).map(([gender, count]) => (
              <div key={gender} className="flex items-center gap-3">
                <div className="w-24 text-xs text-wia-ink/60 shrink-0">{GENDER_LABEL[gender] ?? gender}</div>
                ...
                <div className="w-12 text-xs text-wia-ink/60 text-right shrink-0">
      ...
      {data.dailySeries.length > 0 && (
        <GlassCard className="p-5">
          <h3 className="font-display font-semibold text-wia-ink mb-4">Visitors — last 30 days</h3>
      ...
      {data.totalVisitors === 0 && (
        <div className="py-12 text-center text-wia-ink/50 text-sm">
          No visitor data yet. Analytics will appear once guests start joining.
        </div>
      )}
    </div>
  )
